...
/// MMIO access implementation for AMD SEV-SNP via GHCB exits.
///
...
impl MmioAccess for SvsmHal {
    unsafe fn mmio_read<T: FromBytes>(src: *const T) -> T {
        ...
        // SAFETY: The caller guarantees `src` is a valid, aligned MMIO address.
        unsafe {
    ...
    unsafe fn mmio_write<T: IntoBytes + Immutable>(dst: *mut T, value: T) {
        // SAFETY: The caller guarantees `dst` is a valid, aligned MMIO address.
        unsafe {
            SVSM_PLATFORM
