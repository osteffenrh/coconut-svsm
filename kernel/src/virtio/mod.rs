...
mod zerocopy_wrapper {
    use zerocopy::{IntoBytes, Immutable};
        /// Wrapper that adds IntoBytes and Immutable to types
    /// that don't implement those themselves.
    /// This workaround is unsupported by the zerocopy crate
    /// and might break at any time!
    pub struct Wrapper<'a, T>(pub &'a T);

    /// SAFETY: This wrapper is only used within this implementation
    /// of `Baz::bar` and only on types specific to its usecase.
    unsafe impl<T: Sized> IntoBytes for Wrapper<'_, T> {
    ...
    /// SAFETY: This wrapper is only used within this implementation
    /// of `Baz::bar` and only on types specific to its usecase.
    unsafe impl<T: Sized> Immutable for Wrapper<'_, T> {
        fn only_derive_is_allowed_to_implement_this_trait()
        where
            Self: Sized,
        {
...
unsafe impl virtio_drivers::Hal for SvsmHal {
    ...
    unsafe fn mmio_read<T: Sized + Copy>(src: &T) -> T {
        ...
        cpu::percpu::current_ghcb()
            .mmio_read::<T>(paddr) // putting the wrapper here does not help
    ...
    unsafe fn mmio_write<T: Sized + Copy>(dst: &mut T, v: T) {
        ...
        cpu::percpu::current_ghcb()
            .mmio_write(paddr, &zerocopy_wrapper::Wrapper(&v))
